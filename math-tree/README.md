...
- **Une seule clé nécessaire** : ta propre clé API [Anthropic](https://console.anthropic.com/)
  (Claude, avec vision), saisie dans les réglages de l'appli et stockée
  uniquement dans le `localStorage` de ton navigateur. Elle sert à appeler
  directement `api.anthropic.com` depuis ton appareil — jamais ailleurs.
...
3. Va dans **Réglages** (icône engrenage), colle ta clé API Anthropic, vérifie
   l'identifiant du modèle (mets-le à jour si Anthropic en publie un nouveau),
   puis **Enregistrer**.
...
- **Analyse automatique** : l'image est envoyée à Claude avec l'arborescence
...
- Chaque analyse d'image consomme des tokens sur ta clé API Anthropic (coût à
  ta charge, généralement de l'ordre du centime par image).
